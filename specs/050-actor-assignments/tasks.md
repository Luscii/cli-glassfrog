...
- [ ] **T002** [Shared] Add the `assignments` list render key + `AssignmentsView` + templates — golden + registry-guard tests
...
- [ ] **T003** [Shared] Add the `assignments <actor-id>` command + `assignmentsSeam` + completeness + wiring — RED-first unit tests for every branch
...
- [ ] **T004** [Shared] Make the driving scenarios pass as executable acceptance — new `internal/cli` godog suite over `actor-assignments.feature`; un-`@wip` the behavioral scenarios, keep `@validation` held
