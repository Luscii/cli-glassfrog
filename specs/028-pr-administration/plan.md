...
> **Superseded in part (2026-06-12, by 030-release-drafting ADR-4 — see `.score/memory/DEPRECATION.md`):** the "managed set is *exactly* the seven categories" closedness no longer holds — 030 adds an **eighth** managed label, `no-release-note` (an exclusion marker, applied by the labeler to PRs confined to `specs/`+`.feature`), so the managed *set* is now eight. The seven release-note **categories** are unchanged (the eighth is not a category), and the "no parallel `version:*` namespace" decision still stands. 030's `internal/build` config guard re-pins the new eight-label invariant. The `.github/settings.yml`/`.github/labeler.yml` files (and their "seven managed labels" header comments) keep reading seven until 030 task T001 lands the eighth label.
