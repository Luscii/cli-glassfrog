...
- PR Administration must not create or maintain the label **definitions** (their names, colors, descriptions). **Why**: the managed labels are assumed to already exist in the repository (a one-time maintainer setup); this feature applies and removes labels, it does not own the label catalog.
