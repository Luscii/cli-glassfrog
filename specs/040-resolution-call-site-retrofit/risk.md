...
**Description**: the presence bit threads through 12 `RunE` sites + the 10-declaration `resolveFormat` seam; a missed site would silently keep value-emptiness, so `--base-url ""` fails loud on some commands but falls through on others.
