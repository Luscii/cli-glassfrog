...
2. **Reuse `Project` unchanged; decode the list as `Page[Project]` (016) and the single read as `Document[Project]`.** Chosen.

**Decision**: Option 2. No model change. The list read decodes `glassfrog.Page[Project]`; the single read decodes `glassfrog.Document[Project]`. Both already exist generically — this feature only instantiates them.
...
2. **Commands** — `projects <role-id>` (list: `paging.All[Project]` + `--query`/`-q`, `--status` via `validateStatus`, `--tag`, `--first-page`/`--per-page`, the 025 completeness logic) and `project <proj-id>` (single: `Document[Project]`), guard-registered and wired in `main`; route through `renderResult[T]` and `classifyClientError`.
