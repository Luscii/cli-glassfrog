...
- [x] **T001** [Shared] [P] Add the singular `project` render key + `ProjectView` + templates — golden + registry-guard tests; `internal/render` (new `project.{full,compact}.tmpl`, `ProjectView`, `ResourceProject`) + tests — added `ProjectView`+`ResourceProject`+`ProjectsView`, two templates, 8 golden tests; registry-exhaustiveness guard covers `project`
...
- [x] **T002** [Shared] [P] Add the `projects <role-id>` list command + `projectsSeam` + `--query`/`--status`/`--tag`/`--first-page`/`--per-page` + completeness + wiring — RED-first unit tests for every branch; new `internal/cli/projects.go` + tests + `Assemble()` wiring — new projects.go (seam, list walk, three filters, first-page, completeness), wired in Assemble; 19 unit tests covering walk/empty/filters/status-validation/first-page/mid-walk/classification/structured
...
- [x] **T003** [US2] Add the `project <proj-id>` single-read command — `runProjectGet` + `Document[Project]` decode + the `project` render dispatch — RED-first unit tests; extends `internal/cli/projects.go` + `Assemble()` wiring — added newProjectCommand/runProjectGet (Document[Project] decode, project render dispatch, no list flags), wired in Assemble; 5 unit tests (detail, unknown-id 404, structured raw, list-flag tripwire across 5 flags, ExactArgs)
...
- [x] **T004** [Shared] Make the driving scenarios pass as executable acceptance — new `internal/cli` godog suite over `role-projects.feature`; un-`@wip` the behavioral scenarios, keep `@validation` held — new TestRoleProjectsFeatures suite (Paths names only role-projects.feature), 10 behavioral scenarios un-@wip and passing, 3 @validation scenarios held @wip; reused shared exit-code/stderr/projection step phrasings, helpers return errors not panic
