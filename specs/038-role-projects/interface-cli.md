...
*List (`projects`)* — reuses the **landed `projects` render key** (014) for the human path, unchanged: `full` renders one block per project (`<proj_…>  [<status>]  <description|—>` then an indented `role: … sub-projects: yes/no actions: yes/no tags: …` line); `compact` renders one line per project (`<proj_…>  [<status>]  <description|—>`). For `json`/`yaml`, the walked list emits the **aggregated `{data:[…]}` document** built from each project's raw bytes (per-page `meta` dropped) via `aggregateRawData` — the landed roles/domains/policies walked-list pattern (walk `Page[json.RawMessage]`), **not** a single page's envelope and **not** a decode-and-re-encode of `[]Project`.
...
Errors go to **stderr**; the process exit code is the category from Exit-Code Convention (004), produced through **011's shared `classifyClientError`** (delegating to 031's `Diagnose`). Failures render on stderr regardless of `--output`, exactly as every landed read does today; Output-Aware Failure Rendering (032, specified but not yet landed) will later route structured (`json`/`yaml`) failures to stdout — 038 inherits that automatically when 032 ships, with no edit here. Both commands **introduce no `Outcome` category and no `ExitCode` case**. Every message names the cause **and** a next step, and never includes the token.
...
| Malformed paging mid-walk (`projects`) | `*MalformedPageError` (016) | `RuntimeError` | 1 | the walker's own text — "malformed paging: the cursor did not advance at page N" — surfaced inside the incompleteness note (the partial set is already on stdout) |
...
- **No schema growth, reuses landed generics** (plan ADR-2): unlike 034 (which grew `Policy` and generalized `Document[T]`), Role Projects reuses `internal/glassfrog.Project` (grown by 014) unchanged and instantiates the landed generics: the list walks `Page[json.RawMessage]` (structured) / `Page[Project]` (human), the single read decodes `Document[Project]` (034). No model edit.
...
- **Reuses landed machinery**: the persistent `--base-url` (011) and `--output`/`-o` (020), the shared `classifyClientError` + frozen `Outcome`/`ExitCode` registry (011/015), the `paging.All` walker + `RetryExecutor` (016/017), the walked-list render pattern — `aggregateRawData` for structured + `renderFn` projection for human (018/019/020; `renderResult[T]` is the single-page `/me*` dispatch and is **not** used by a walked list), and for the single read `output.RenderSuccess` (structured) + `renderFn` (human) — and the landed diagnostic chain (031 — `Diagnose`/`reportClientError`, stderr). Adds only the two commands and the singular `project` render path. (Output-Aware Failure Rendering (032) is specified but not yet landed; when it ships, 038's failures gain `--output`-aware rendering for free — it is a future enhancement, not a dependency 038 reuses today.)
