...
- **Observation (not a matrix finding) — stdin read-cap overflow unspecified**: plan ADR-4 and interface-spec introduce a bounded stdin read (`maxStdinBytes`), but neither the spec nor the interface specifies what happens when piped input *exceeds* the cap (error vs. silent truncation). This is a shared *silence*, not a cross-artifact contradiction, so no C/K/H check fails on it. Given Constitution VI's "never silently truncate" sensitivity, recommend specifying the overflow behaviour (preferably: error, naming the bound) before or during implement (T005). Carried over from the checklist run. **Resolved by 04c92d9 (PR #82 triage): the spec STDIN accord, the interface `FromStdin` docstring + Error Communication table, and the T005 acceptance criteria now specify "exceeds `maxStdinBytes` → error, no silent truncation" (Constitution VI). The reviewer independently re-raised this observation before the fix landed.**
