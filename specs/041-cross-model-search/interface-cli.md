...
**Dispatch**: `search`'s `RunE`, before any network call, in order: (1) cobra `Args` (`ExactArgs(1)`) — a missing or extra positional is a usage error; (2) `--types` value validation against the closed 8-value set; (3) `--output` resolution (020). Any failure here is a fail-fast usage error and **no request is sent** (a transport tripwire asserts this, per 011/013/025/026).
