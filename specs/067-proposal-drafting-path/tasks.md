...
- [x] **T002** [Shared] Add the best-effort drift-guard test in `internal/build` — pins the composed leaves AND the gated-membership invariant (the inverse of 066's disjointness) — 1 validation scenario + standalone guard; gated-membership derived as "the single composed leaf 063 gates" (no hard-coded write-leaf); empirically confirmed to bite on both regression directions
