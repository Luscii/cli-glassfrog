...
- **Path-swap is the only data difference** (plan ADR-3): the runner reuses the landed `runActorsList` shape (kind validation → `paging.All` walk → render → completeness note) with the request path set to `/roles/{role_id}/subroles/actors` and the role id supplied from the positional. Whether that is expressed by parameterizing `runActorsList` with the path + base query or by a thin sibling runner is implementation-level; parameterizing keeps the kind/paging/render/error logic single-sourced.
...
- **Completeness, errors, config reuse the shared seams unchanged** (plan Cross-cutting): `paging.All` + `--first-page` (016/025 ADR-3), the shared `classifyClientError`/frozen `Outcome`/`ExitCode` registry (011/015), 032's `reportFailure` chokepoint, the 035-widened render flow, the persistent `--base-url`/`--output` flags — all reused exactly as `runActorsList` uses them. Adds no new `Outcome`/`ExitCode`.
