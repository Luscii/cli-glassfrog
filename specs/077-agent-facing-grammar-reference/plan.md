...
**Configuration**: only the output-format resolution chain participates (flag → env → `.glassfrogrc` → default). Token and base URL are never resolved — not "resolved and unused," never resolved, so a malformed credential *value* cannot block the command. It does not follow that a malformed `.glassfrogrc` cannot: that file carries the `output` setting as well as the credential, and 020/040 fail loud on a file that does not parse rather than falling through to the default. The command keeps the shared conduct (interface-cli § Interactions records the boundary).
