...
- **Committed grammar artifact** — one JSON file in the grammar package: the vocabulary entries (type + placement class, provenance `published contract`) and the fact entries (id, shape, wrong shape, disposition, symptom, provenance `empirical observation`). Header-marked as generated; never hand-edited.
