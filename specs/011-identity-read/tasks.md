...
- [x] **T003** [Shared] [P] Register `--base-url` as a persistent flag on the root command; update the 003 help-regression tests — RED-first — registered in NewRootCommand (root construction) under apiclient.FlagBaseURL; added a 003 positive pin (root help shows --base-url) and a root inheritance/retrievability test; all existing help/dispatch tests still pass (additive flag)
...
- [x] **T004** [Shared] Add the `me` command — `newMeCommand(seam)` + pure `runMe`/`formatMe`/`validateInclude` over an injected seam — RED-first unit tests for every branch — me.go (command + pure runMe/formatMe/validateInclude + productionSeam meSeam methods); dispatch.go gained an outcomeError carrier so APIError(3)/NetworkUnavailable(6) reach the exit code (the runLogin pattern can't surface them); 19 unit tests over a fake transport cover every branch incl. tripwire (no request on usage), token-never-in-output, and outcome→exit-code mapping
...
- [x] **T005** [Shared] Wire `me` into the root in `Assemble()`, and make the driving scenarios pass as executable acceptance via a new `internal/cli` godog suite — one MustRegister line in Assemble; new TestIdentityReadFeatures suite scoped to only identity-read.feature, 11 behavioral scenarios pass (57 steps), 4 @validation stay @wip; step helpers return errors; token-never-in-output enforced in the When step across every scenario
