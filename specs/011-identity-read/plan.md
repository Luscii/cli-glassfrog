...
- **Request Execution (010, `internal/apiclient` — upstream dependency, NOT YET IMPLEMENTED)**: `me` calls `NewClientFromOS(ctx)` then `(*Client).Execute(reqCtx, Request{Method:"GET", Path:"/me", Query:{include?}}, &me)`. 011 consumes 010's `Client`/`Execute`/`Request`/`Response` and the typed errors **unchanged**; it adds nothing to `apiclient`. *Because 010 is at "Analyzed" (designed, not built), 011's implementation is gated on 010 landing — see Implementation Strategy and Risks.*
...
Four phases, mostly linear. **Hard dependency**: 010 (`NewClientFromOS`, `Execute`, `Request`, `Response`, `*TransportError`/`*ResponseError`/`*DecodeError`, propagated `*AuthError`) must be implemented before Phase 3 can compile against it. 010 is at "Analyzed" (designed, pre-implementation guard passed); 011 should be built on a branch that has 010's `apiclient` code, or 010 lands first. Phases 1–2 have **no** 010 dependency and can proceed immediately.
...
- **Phase 2 — `Outcome`/`ExitCode` extension + `classifyClientError` + persistent `--base-url` flag**: add `NetworkUnavailable`/`APIError` to the `Outcome` enum and `ExitCode` cases (codes 6/3); add the shared `classifyClientError(err) Outcome` (`errors.As` chain over 010's typed error types — referenced by type even before 010's bodies exist, since the types are pinned by 010's interface-spec); register `--base-url` as a persistent root flag in the wiring layer and update the affected 003 help tests + the exit-code pin tests. RED-first: classifier table test with exhaustiveness guard; `ExitCode` pins for the two new codes; root-help reflects the new global flag. *Depends on: 010's error type names (interface-spec); does not need 010's behavior.*
...
- **010 not yet implemented when 011 starts** (high likelihood, medium impact): Phase 3 cannot compile against `apiclient.Execute` until 010's bodies land. *Mitigation*: Phases 1–2 (schema, exit-code extension, classifier, flag) have no 010 dependency and proceed now; Phase 3 sequences after 010 is implemented (or 011 builds on a branch including 010). The tasks skill makes the dependency explicit so the Builder does not start Phase 3 against missing code.
