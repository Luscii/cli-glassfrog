...
- [x] **T001** [Shared] Add the `Request.IfMatch` field and its conditional `If-Match` send in `Execute`, with unit tests — 9 scenarios (2 @validation held @wip), 5 send unit tests; no LEARNINGS, no drift — 7 @wip removed (7 BDD scenarios pass), 5 send unit tests added; no LEARNINGS, no drift
