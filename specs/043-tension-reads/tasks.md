...
- [x] **T005** [Shared] Make the driving scenarios pass as executable acceptance — new `internal/cli` godog suite over `tension-reads.feature`; un-`@wip` the behavioral scenarios, keep `@validation` held — TestTensionReadsFeatures (Paths → tension-reads.feature only); 10 behavioral scenarios pass, 3 @validation held @wip
