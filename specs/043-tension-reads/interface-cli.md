...
*List (`tension list`)* — the **new plural `tensions` render key** (plan ADR-2): `full` renders one block per tension (`<ten_…>  [<status>]  <label|—>` then an indented `body: <body summary> sensing role: <role_…|—>` line); `compact` renders one line per tension (`<ten_…>  [<status>]  <label | body summary>`). For `json`/`yaml`, the walked list emits the **aggregated `{data:[…]}` document** built from each tension's raw bytes (per-page `meta` dropped) via `aggregateRawData` — the landed roles/domains/policies/projects walked-list pattern (walk `Page[json.RawMessage]`), **not** a single page's envelope and **not** a decode-and-re-encode of `[]Tension`.
