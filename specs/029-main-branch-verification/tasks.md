...
- [x] **T002** [US1] Add `.github/workflows/main-verify.yml` triggered by `push` to `main`, calling the reusable test workflow — actionlint clean; SHA-keyed concurrency + cancel-in-progress: false (ADR-4); no lint/aggregation/branch-protection (ADR-3); removed @wip from 6 behavioral scenarios (1 @validation held for /score:validate) — the post-merge net that re-runs the suite on every merge and surfaces a regression as a red run + failing commit status, with no enforcement and no cancelling of in-flight runs
