...
- [ ] **T001** [Shared] Add `.github/workflows/test.yml` (reusable, `on: workflow_call`) holding the OS test matrix, and refactor `.github/workflows/ci.yml`'s `test` job into a `uses:` call to it — the test matrix becomes a single source of truth shared by the pre-merge gate (024) and the post-merge net (029)
