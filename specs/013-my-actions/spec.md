...
My Actions is one of the four **Self-Service Reads** (problem: *read "what's mine" — me, my roles, actions, and projects*). It is a read command that lists the **actions owned by roles the authenticated practitioner fills** — token-scoped through `GET /me/actions` (`listMyActions`), not the org-wide action surface. It answers "what next-actions are on my plate?" for the practitioner the API key resolves to.

It sits on the proven read chain: identity rides through Request Authentication (007), and the call goes out through Request Execution (010), the single transport seam. It is a sibling of Identity Read (011), My Roles (012), and My Projects (014) and shares their `/me*` read-presentation convention. It is deliberately narrow: it fetches **one page**, applies an optional **status filter** validated against the spec's vocabulary, and renders the result through the shared projection. It does not walk pages (Pagination, 016), does not interpret non-2xx responses (API Error Extraction, 015), and does not back off on `429` (Rate-Limit Handling, 017) — those siblings own that work and build on what Request Execution surfaces.
...
- When invoked, the system requests the authenticated actor's actions from `GET /me/actions` through Request Execution, so the call carries the resolved identity without this command attaching the header or re-resolving the connection.
- When the API returns a successful response, the system renders the actions it received through the shared `/me*` read-presentation projection — a reshaped, summarized view, not the raw API payload.
...
- When invoked with no status filter, the system requests the actor's actions without a status constraint.
...
- When the practitioner owns no actions, or none match the supplied status filter, the system reports success with an empty list — zero matches is a valid answer, not a failure.
...
- The system must not send a `--status` value the spec's vocabulary does not define. **Why**: an unsupported filter is a usage error the contract will refuse; catching it locally (as Identity Read, 011, does for `--include`) gives the agent operator a usage-error exit and the supported set to self-correct, rather than an opaque API error — and `GET /me/actions` does not even document a `400` for a bad status, so pass-through could silently return wrong results.
...
- The system must not read the org-wide action surface. **Why**: this is the *self-service* "what's mine" read — it is token-scoped to `GET /me/actions`, and reaching past that would exceed the caller's self-service intent.
- The system must not create, update, or otherwise mutate actions. **Why**: standalone operational writes are out of scope for the project's read + propose core; this is a read.
...
- **Request Execution (010 — upstream dependency)**: the seam this command sends its single `GET /me/actions` request through and reads the response or typed error from. This command re-resolves nothing and makes no transport decisions of its own.
...
- **Glassfrog API — `GET /me/actions` (`listMyActions`)**: returns a paginated list of actions owned by roles the requester fills (primary assignments), with an optional `status` query filter. The API owns permission scoping; the status vocabulary is the spec's enum the command validates against.
...
- **My Roles (012) — sibling**: the first paginated `/me*` list; establishes the "more results available" signal convention this command's first-page boundary follows. My Projects (014) is this command's twin over a different resource.
...
**Scenario: list the practitioner's actions**
Given a connection context with a usable base URL and a present token
When the my-actions command is invoked with no status filter
Then the system requests `GET /me/actions` through Request Execution
And renders the first page of returned actions through the shared `/me*` projection.
...
And only the actions the API returns for that filter are rendered.

**Scenario: more results than one page**
Given the practitioner owns more actions than fit on one page
...
Given the practitioner owns no actions, or none match the supplied status filter
When the command is invoked
...
When the command runs
Then exactly one `GET /me/actions` request is made
