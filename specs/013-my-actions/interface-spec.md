...
This accord pins the **Go API surface** My Actions introduces: the `Action` resource model added to `internal/glassfrog`, the shared `validateStatus` + status set, and the `my actions` command's internal shape (the injected seam, the pure `runMyActions`/`formatMyActions`). It **reuses** — and does not redefine — 011's `internal/glassfrog` foundation, `classifyClientError`, `Outcome`/`ExitCode`, and persistent `--base-url`, and 012's `my` parent command, `Pagination` type, list envelope, and signal renderer. The CLI-facing surface (command, flags, projection, exit codes) is in `013/interface-cli.md`; this file is the package-level contract the Builder implements and the Verifier tests against. Field names and concrete Go types are a build detail; the **shapes, signatures, and which fields are projected** are the contract.
...
- **From `010/interface-spec.md`**: `apiclient.AssembleFromOS(flagValue)`, `apiclient.NewClientFromOS(ctx)`, `(*Client).Execute(reqCtx, Request, out)`, the `apiclient.Request` descriptor, and the typed errors `*AuthError` / `*TransportError` / `*ResponseError` / `*DecodeError`. `my actions` adds nothing to `apiclient`.
- **From `011/interface-spec.md`**: `classifyClientError(err) Outcome`, the `Outcome` enum (incl. `NetworkUnavailable`/`APIError`), the `ExitCode` mapper (codes `3`/`6`), and the persistent `--base-url` root flag (`apiclient.FlagBaseURL`). `my actions` reuses these; it adds no `Outcome` case and no `ExitCode` case.
...
- **Build-once / send-once flow**: `runMyActions` calls `seam.assemble` once and `seam.newClient` once, then `Execute` once. Resolution already happened at assembly (009); `my actions` re-resolves nothing and never reads `ctx.Cred.Token`.
...
- **Seam injection**: the command is constructed with a seam (`newMyActionsCommand(productionSeam{})` under `my` in `Assemble()`); tests construct it with a fake seam binding a fake `http.RoundTripper`, so every branch runs offline and off the real `~/.glassfrogrc`.
...
- **No secret anywhere**: none of the messages or the projection renders the token; `my actions` never reads `ctx.Cred.Token`. Pinned across success and every error branch.
...
- **Reuses 011 unchanged**: `my actions` is the first command to *consume* 011's read-surface foundation (the `glassfrog` package, `classifyClientError`, `Outcome`/`ExitCode`, `--base-url`) without extending it — proof the 011 surface is reusable as designed. It adds no exit code and no classifier branch.
- **Reuses 012's list foundation**: the `my` parent, `Pagination`, the envelope, and the signal renderer. The `actions` leaf is a sibling of the `roles` leaf. If 013 lands before 012, it creates the shared `Pagination`/envelope/`my`-parent in place and 012 reuses them (first-to-land-creates; 005/006/007 pattern).
...
- **No new configuration of its own**: `my actions` introduces no env var or `.glassfrogrc` key; `--base-url`/`GLASSFROG_BASE_URL`/`base_url` are owned upstream (008/011), `GLASSFROG_TOKEN`/`token` by 005. `--status` is a request-shaping flag, not configuration.
