...
- **Phase 3 — Command wiring + executable acceptance**: register the guard-validated leaf (with its `Args` validator and `Short`), wire it in `main`, classify outcomes into the existing `Outcome` categories. godog step definitions for the 006 driving scenarios (arg→home, stdin→CWD, env-persist, interactive-prompt, unwritable→write error, malformed→format error, blank-token, non-interactive-no-token, existing-token-no-override, interactive-location-choice) in `features/unauthenticated-access/credential-storage.feature`, reusing existing step vocabulary and driving the interactive paths through the injected `isTTY` seam. *Depends on: Phases 1–2.*
