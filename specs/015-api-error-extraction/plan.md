...
- **`internal/cli` — the consumer wiring (maps + presents).** The three shared helpers 011 introduced grow to understand the refined error:
  ...
  - `classifyClientError` branches the API error on its status: **401/403 → `PermissionError`** (taking the reserved code 4), everything else (including 429) → `APIError` (code 3). This adds `PermissionError` to the `Outcome` enum + its `ExitCode` case. (429 stays `APIError`(3) in this slice — see ADR-3 on the deferred 429→rate-limit(5) split.)
...
internal/cli                              (the consumer — maps + presents; 011's shared helpers, grown)
  ...
  classifyClientError(err) Outcome                        ── the ONE shared chain (011), grown  [ADR-3]
    ...
    ├─ *ProblemError / *ResponseError  → status 401|403 → PermissionError(4)   [the split — reserved code 4]
    │                                else            → APIError(3)         (incl. 429 — 015 does not split it; see ADR-3)
...
**Decision**: Option 1. `classifyClientError` gains the 401/403→`PermissionError`(4) split (checked on the `*ProblemError`/`*ResponseError` status, **before** the generic `*ResponseError`→`APIError` arm given the wrapping in ADR-1); `PermissionError` joins the `Outcome` enum (`dispatch.go`, with its `String()` arm) and `ExitCode` (`exitcode.go`). **429 stays `APIError`(3)** in this slice. Rate-Limit Handling (017, landed) owns the 429 *retry/backoff* but its spec Non-Behavior explicitly forbids it from *classifying* the 429 — it defers the 429→rate-limit(5) exit-code split back to API Error Extraction (015) as "the producer that types non-2xx responses" (017 DECISIONS, 2026-06-07). **Whether 015 takes on that 429→`RateLimited`(5) split is an open scope decision** (flagged in PR #37); this plan currently leaves 429 at `APIError`(3) — code 5 stays reserved. 403 plan-limit *messaging* stays with Unsignalled Plan Limits (spec Non-Behaviors). No code is renumbered. The classifier's table test grows with the new mapping and keeps its `len`+comma-ok exhaustiveness guard (PR #10 LEARNINGS).

**Consequences**: The reserved code 4 is filled by its forecast producer; the `apiclient` capability stays exit-code-free (honoring the spec). 011–014 are unchanged except 401/403 now exits 4 (previously 3) — a published, reserved code, not a renumber. `outcomeToDispatchError`'s existing `default` arm already routes a non-Success/Usage/Runtime `Outcome` through `*outcomeError`, so `PermissionError` reaches `ExitCode` with no dispatch edit. *Precedent-setting: an API-response-interpretation capability fills its reserved exit code by splitting the generic `APIError`(3) at the shared `classifyClientError` registry on the status its `apiclient` type carries — the same shape would apply to a future 429→rate-limit(5) split (which landed 017 defers to 015; see the open scope note above).*
...
- **429→rate-limit(5) ownership is unresolved across 015 and 017** (medium likelihood, low impact): landed 017 does the retry/backoff but defers the 429→rate-limit(5) *classification* to 015, while this slice leaves 429 at `APIError`(3) — so code 5 has no producer until the scope decision is made (flagged in PR #37). *Mitigation*: 429→`APIError`(3) is explicitly retained and pinned by a test for now; the "Does Not Cover" section and ADR-3 record the open decision so code 5's owner is chosen deliberately, not dropped.
...
- **429 backoff/retry** — Rate-Limit Handling (017, landed). 015 extracts a 429 into a typed error like any other non-2xx and leaves it at `APIError`(3). **The 429→rate-limit(5) exit-code split is *unassigned*** — landed 017 defers it to 015, but this slice does not implement it (an open scope decision flagged in PR #37; ADR-3).
