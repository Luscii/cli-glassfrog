...
API Error Extraction is the capability that makes a non-2xx Glassfrog response **legible** — it pairs with the Next-tier *Opaque Failures* problem ("when a call fails, the caller can't tell what went wrong or what to do next"). Request Execution (010) deliberately stops at a **generic, uncategorized** `*apiclient.ResponseError{StatusCode, Header, Body}` and never interprets it (010 ADR-3). 015 takes exactly that value and refines it into a typed, code-free `*apiclient.ProblemError` carrying the authoritative HTTP status, the API's RFC 9457 Problem Details (`type`/`title`/`detail`), and the raw body/headers — so the operator sees the API's own cause instead of a bare status number, and the consuming command can branch on the status to assign the right exit code.

The work splits across the two packages the read surface already uses, with no new package:

- **`internal/apiclient` — the extraction capability (produces, decides no exit code).** A new typed error `ProblemError` that **wraps** the originating `*ResponseError` (so `errors.As(err, &ResponseError)` still matches and the raw carrier stays reachable), plus a pure `ExtractProblem(*ResponseError) *ProblemError`. The function best-effort-parses the body as RFC 9457 Problem Details (`application/problem+json`: `type`, `title`, `status`, `detail`), **degrades gracefully** when the body isn't readable (empty, non-JSON, HTML from a gateway, or JSON missing the required members) by deriving a fallback detail from the HTTP status and keeping the raw body. It carries **two provenance/metadata fields** so the consumer can act on the result without re-parsing: a **`DetailSynthesized` flag** (false = the detail is the API's own; true = it's the status-derived fallback) and a **`BodyStatus`** holding the body's own `status` member when present — because the **HTTP status is authoritative** (the body's `status` is metadata only and never overrides). It never returns an error of its own and never reads the request token (it touches only the response-side `ResponseError`).
- **`internal/cli` — the consumer wiring (maps + presents).** The three shared helpers 011 introduced grow to understand the refined error:
  - `reportClientError` (the read-surface chokepoint that 011–014 all funnel through) **refines** a `*ResponseError` into a `*ProblemError` via `ExtractProblem` once, so the typed error travels up the chain and both the message and the category are computed from it — no per-command edits.
  - `formatClientErrorMessage` surfaces the API's `detail` (and `title`) when `DetailSynthesized` is false, falling back to the existing "the API returned a non-2xx response: status N" wording when it is true (keyed on the provenance flag, **not** on detail emptiness — the fallback always fills `detail`). It also **appends a next-step hint** (≥ for 401/403: "check the token's access / membership") so the arm matches its siblings and CONSTITUTION II.
  - `classifyClientError` branches the API error on its status: **401/403 → `PermissionError`** (reserved code 4), **429 → `RateLimited`** (reserved code 5), everything else → `APIError` (code 3). This adds `PermissionError` and `RateLimited` to the `Outcome` enum + their `ExitCode` cases.
...
internal/cli                              (the consumer — maps + presents; 011's shared helpers, grown)
  ...
  reportClientError(stderr, err)
    │  if errors.As(err, &re) && not already *ProblemError → err = apiclient.ExtractProblem(re)   [ADR-4]
    │  Fprintln(stderr, formatClientErrorMessage(err))    ── surfaces .Detail / .Title, fallback "status N"  [ADR-4]
    └─ return classifyClientError(err), err               ── the typed *ProblemError travels up the chain
  ...
  classifyClientError(err) Outcome                        ── the ONE shared chain (011), grown  [ADR-3]
    ...
    ├─ *ProblemError / *ResponseError  → branch on status:   [the split — reserved codes 4, 5]
    │                                    401|403 → PermissionError(4)
    │                                    429     → RateLimited(5)
    │                                    else    → APIError(3)
...
### ADR-1: API Error Extraction is a pure `internal/apiclient` capability that refines 010's `*ResponseError` into a typed code-free `*ProblemError` wrapping it
...
**Decision**: Option 1. A pure `ExtractProblem(*ResponseError) *ProblemError` in `internal/apiclient`; `ProblemError` wraps the `*ResponseError`. Because it wraps, the consumer's classifier must check `*ProblemError` (or branch on status) **before** the bare `*ResponseError` arm — the same discrimination-order discipline 010/011 already apply for `*AuthError` before `*TransportError`. The interface pinned the produced type as `ProblemError` (deliberately distinct from the `cli.APIError` *Outcome* it mostly maps to); field names and signature are interface-level.

**Consequences**: The non-2xx carrier and its interpretation stay cleanly split across 010 and 015, each in `apiclient`. The typed error is `errors.As`-able and carries everything a programmatic caller needs (status, detail, the raw body, and the response headers). `apiclient` still imports nothing from `cli`. *Precedent-setting: a non-2xx is refined by a pure `apiclient` function into a typed error that wraps 010's `ResponseError`; later response-interpretation capabilities (017) follow the same refine-the-carrier shape.*

### ADR-2: `ExtractProblem` degrades gracefully and treats the HTTP status as authoritative (resolves spec forks 1 & 2)
...
**Options considered**:
1. **Best-effort parse + status-derived fallback; HTTP (ResponseError) status authoritative.** Attempt to decode the body as Problem Details regardless of `Content-Type`; on empty/non-JSON/missing-members, set a fallback detail from the status (Go's `http.StatusText`) and keep the raw body; ignore a disagreeing body `status` for the authoritative field (capture it as metadata). `ExtractProblem` returns no error.
...
**Decision**: Option 1. `ExtractProblem` parses best-effort, degrades to a `http.StatusText`-derived fallback detail (the exact wording `[ASSUMED]`, interface-level), preserves the raw body on every path, and sets the authoritative `StatusCode` from the `ResponseError`. Two fields make the result self-describing so the consumer needn't re-parse: **`DetailSynthesized`** (true when the detail is the fallback, false when it's the API's own) and **`BodyStatus`** (the body's own `status` member when present) — the body's `status` is captured here as metadata and never overrides the authoritative `StatusCode`.

**Consequences**: A failed call is never opaque and never panics on a junk body (the empty/HTML/missing-members edges all yield a usable typed error). The function is total (returns `*ProblemError`, no error), which keeps the consumer flow branch-free. `DetailSynthesized` lets `formatClientErrorMessage` key its fallback on provenance rather than on `detail` emptiness (the fallback always fills `detail`), and resolves the No-Fabricated-Data concern — a synthesized detail is never presented as the API's own words. `BodyStatus` makes the spec/feature "carried as metadata only" assertion observable. The fallback wording is a tunable detail, not a behavioral contract. *Feature-local; the totality + fail-soft-on-parse discipline is the notable property.*
...
2. **Bucket statuses into named categories inside the `apiclient` `ProblemError` type** — rejected: spec fork 3 chose "stay narrow"; and exit-code mapping is `cli`'s concern per 004/011, not `apiclient`'s (which would mean `apiclient` reaching toward exit codes).
...
**Decision**: Option 1. `classifyClientError` gains the status split (checked on the `*ProblemError`/`*ResponseError` status, **before** the generic `*ResponseError`→`APIError` arm given the wrapping in ADR-1): **401/403 → `PermissionError`(4)** and **429 → `RateLimited`(5)**, everything else → `APIError`(3). `PermissionError` and `RateLimited` join the `Outcome` enum (`dispatch.go`, with their `String()` arms) and `ExitCode` (`exitcode.go`, taking reserved codes 4 and 5). The 429→rate-limit(5) ownership is settled here: Rate-Limit Handling (017, landed) owns the 429 *retry/backoff* but its spec Non-Behavior forbids it from *classifying* the 429 and defers that to API Error Extraction (015) as "the producer that types non-2xx responses" (017 DECISIONS, 2026-06-07) — so **015 classifies (exit 5), 017 handles (retry); 015 never backs off.** 403 plan-limit *messaging* stays with Unsignalled Plan Limits (spec Non-Behaviors). No code is renumbered. The classifier's table test grows with the two new mappings and keeps its `len`+comma-ok exhaustiveness guard (PR #10 LEARNINGS).

**Consequences**: The reserved codes 4 and 5 are filled by their forecast producer; the `apiclient` capability stays exit-code-free (honoring the spec). 011–014 are unchanged except 401/403 now exits 4 and 429 now exits 5 (both previously 3) — published, reserved codes, not a renumber. `outcomeToDispatchError`'s existing `default` arm already routes a non-Success/Usage/Runtime `Outcome` through `*outcomeError`, so `PermissionError` and `RateLimited` reach `ExitCode` with no dispatch edit. *Precedent-setting: an API-response-interpretation capability fills its reserved exit codes by splitting the generic `APIError`(3) at the shared `classifyClientError` registry on the status its `apiclient` type carries; 015 owns both the 401/403→permission(4) and 429→rate-limit(5) splits, with 017 handling the 429 retry/backoff above the seam.*
...
**Options considered**:
1. **Enrich once at `reportClientError`.** Before format+classify, refine a `*ResponseError` into a `*ProblemError` via `ExtractProblem`; pass the typed error to both helpers, so the message surfaces `.Detail`/`.Title` (fallback "status N") and the returned error that travels up the chain IS the typed `*ProblemError`. One shared edit; all reads (011–014) benefit; the category and the message are computed from the same typed value (they can never disagree — the `reportClientError` contract).
...
3. **Parse independently inside `classifyClientError` and `formatClientErrorMessage`** — rejected: classification needs only the status (which `ResponseError` already carries — no parse needed), so this double-parses; and the error returned to the caller would stay the raw `*ResponseError`, not the typed `*ProblemError` the spec says the system "reports ... to the calling command."

**Decision**: Option 1. `reportClientError` refines `*ResponseError → *ProblemError` once (guarding against double-refinement when the error is already an `*ProblemError`); `formatClientErrorMessage` gains an `*ProblemError` arm that surfaces the detail (and title) with the "status N" fallback; the returned error is the typed `*ProblemError`. `classifyClientError` reads the same typed value (ADR-3).
...
**Secret hygiene (CONSTITUTION II)**: 015 touches only the **response** side — `ExtractProblem` reads `ResponseError`'s status, headers, and body (the API's reply); the `X-Auth-Token` is a *request* header and is never present. No 015 output (`ProblemError.Error()`, the surfaced detail/title, the fallback) can carry the token. Pinned by a test asserting the typed error and the rendered message never contain a token-shaped value, mirroring 010's token-never-in-output test.

**Error handling (CONSTITUTION III)**: the capability is **total and fail-soft on the parse** but the overall flow still **fails loud** — `ExtractProblem` never returns an error and never panics on a junk body (it degrades), yet the result is always a typed error that the consumer maps to a non-zero exit code (401/403→4, else→3) and a non-empty message. A malformed body never becomes a silent success (the input is already a non-2xx). The body `status` mismatch is resolved deterministically (HTTP status wins, ADR-2).

**Testing (CONSTITUTION IV)**: RED-first, hermetic, no real network. `ExtractProblem` is unit-tested table-driven over crafted `ResponseError` values: a valid Problem Details body (detail/title/type extracted); extension members (standard four surfaced, raw body retained); an empty body (status fallback); a non-JSON/HTML gateway body (status fallback, no Content-Type gate); JSON missing required members (fallback); a body `status` that disagrees with the HTTP status (HTTP authoritative). The consumer wiring is tested at `internal/cli`: the `classifyClientError` table test grows with 401/403→`PermissionError`, 429→`RateLimited`, and a 500→`APIError` row, keeping the `len`+comma-ok exhaustiveness guard (PR #10 LEARNINGS); `exitcode_test.go` adds the now-live `PermissionError`→4 and `RateLimited`→5 mappings (codes 4 and 5 were already pinned constants); `formatClientErrorMessage` tests assert detail-surfaced, the "status N" fallback, and the permission/rate-limit next-step hints. The driving scenarios become a **new `internal/cli` godog suite** pointed at its **own** feature file `features/opaque-failures/api-error-extraction.feature` (LEARNINGS — never the whole `features/` dir); step helpers return errors, never panic; reuse the existing read-surface step vocabulary where an assertion already exists (grep the package's `sc.Step(` registrations first).
...
- **Phase 1 — the `apiclient` extraction capability**: define `ProblemError` (wraps `*ResponseError`; carries the authoritative `StatusCode` + `Type`/`Title`/`Detail` + `DetailSynthesized` + `BodyStatus`; exposes `Body`/`Header` via the wrapped value) and the pure `ExtractProblem(*ResponseError) *ProblemError` (best-effort RFC 9457 parse, no Content-Type gate, status-derived fallback, HTTP-status-authoritative, total/never-errors). RED-first table-driven unit tests over crafted bodies (valid / extensions / empty / HTML / missing-members / status-mismatch). Purely additive; no existing `apiclient` file changed. *Depends on: 010 (landed).*
- **Phase 2 — the consumer wiring (`internal/cli`)**: add `PermissionError` and `RateLimited` to the `Outcome` enum (`dispatch.go`) + their `String()` arms and their `ExitCode` cases (`exitcode.go`, taking codes 4 and 5); split `classifyClientError` (401/403→`PermissionError`, 429→`RateLimited`, checked before the generic `*ResponseError` arm; else→`APIError`); enrich `reportClientError` to refine `*ResponseError`→`*ProblemError` once; grow `formatClientErrorMessage` to surface `.Detail`/`.Title` with the "status N" fallback and the permission/rate-limit next-step hints. Update the classifier table test (+ exhaustiveness guard), `exitcode_test.go` (codes 4 and 5 now live), and the message tests; confirm 011–014's existing tests still pass (only 401/403→4 and 429→5 change exit codes, both from 3). *Depends on: Phase 1.*
- **Phase 3 — executable acceptance**: godog step definitions for the driving scenarios (valid extraction; 404 detail; extension preservation; empty-body degradation; non-JSON degradation; status-mismatch authority; 429-extracted-not-backed-off; 403-carried-generically; the consumer exit-code mappings 401/403→4, 429→5, default→3) in a new `internal/cli` suite pointed at `features/opaque-failures/api-error-extraction.feature`. *Depends on: Phase 2.*

---

## Risks

- **Wrapping breaks the classifier discrimination order** (medium likelihood, medium impact): `ProblemError` wraps `*ResponseError`, so `errors.As(err, &responseErr)` matches an `*ProblemError` too; if the classifier checks the bare `*ResponseError` arm first, every 401/403 falls through to `APIError`(3) and the split silently never fires. *Mitigation*: branch on status in the `*ProblemError`/`*ResponseError` arm itself (one arm, status-driven) rather than two ordered arms — or check `*ProblemError` first; pin a test asserting a 401 maps to `PermissionError`(4), not `APIError`(3).
...
- **429→rate-limit(5) classification must stay consistent with the 401/403 split** (low likelihood, low impact): 015 now owns both splits (ownership resolved — landed 017 defers the 429 classification to 015; ADR-3). The risk is purely that the 429 arm shares the same wrapping/discrimination-order hazard as the permission arm. *Mitigation*: both arms are one status-driven check before the generic `*ResponseError` arm; a test pins 429→`RateLimited`(5) (not `APIError`(3)) alongside the 401→`PermissionError`(4) test.
- **Malformed/junk body crashes extraction** (low likelihood, high impact): an empty, HTML, or truncated body must not panic. *Mitigation*: `ExtractProblem` is total (ADR-2) — every parse failure degrades to the status fallback; the empty/HTML/missing-members edges are explicit table-test rows.
...
- **429 backoff/retry/`Retry-After` handling** — Rate-Limit Handling (017, landed) owns it, above the `Execute` seam. 015 does the 429 *classification* (→ `RateLimited`(5), ADR-3) but never retries or sleeps; the two compose (010 sends → 017 retries → 015 classifies the final outcome).
...
- **Following pagination** — Pagination (016, landed), which reads paging state from the response **body** (`meta.pagination`), not from headers; 015 only refines the *error* path and leaves the 2xx body/headers reachable on the carrier.
- **Structured `--output json` error rendering** — a deferred cross-cutting output flag (a Non-Behavior on the read surface since 011).
- **The exact field names and the `ExtractProblem` signature** — `/score:interface` pinned them (the produced type is `ProblemError`, deliberately distinct from the `cli.APIError` *Outcome*).
