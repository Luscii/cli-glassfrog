...
- The system must not retry, back off, sleep, or treat `429` specially. **Why**: Rate-Limit Handling (017) owns backoff; a `429` is extracted into a typed error like any other non-2xx (carrying its rate-limit headers) so 017 can read and act on it.
...
- **Rate-Limit Handling (017 — downstream sibling)**: reads the `429` status and rate-limit headers to back off. API Error Extraction extracts a `429` into a typed error like any other non-2xx and does not back off, so 017 can act on the raw rate-limit signal.
