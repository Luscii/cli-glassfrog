...
- [x] **T001** [Shared] [P] Add the caller-template engine to `internal/render` — `UserTemplate` parsed into a clone of the built-in set, `ParseUserTemplate`, `(*UserTemplate).Render`, and the typed `UserTemplateError` (parse vs. execute). — 11 unit/golden tests; parse-into-clone via a never-executed `userTemplateBase` (Clone-after-execute safe); StageParse/StageExecute.
...
- [x] **T002** [Shared] [P] Add the discriminated selection to `internal/output` — `TemplateRef`, `Selection`, `ResolveSelection`/`ResolveSelectionFromOS`, with the flag-only template-ref rule and `stdin` added to the reserved words. — 8 pure-core tests (flag tokens/stdin/file, reserved-wins, env/file non-token → FormatError, precedence, rcfile read error); flag rung delegates env/file to ResolveFormat unchanged.
...
- [x] **T003** [Shared] [P] Map `*render.UserTemplateError` to `UsageError` in `classifyClientError` — a new `errors.As` arm beside the `*output.FormatError` arm; unit test pins parse and execute stages both classify usage. — arm added to `Diagnose` (diagnostic.go) before the fail-safe; `TestClassifyClientError_UserTemplateError` pins both stages → UsageError and the built-in `*RenderError` staying RuntimeError.
...
- [x] **T004** [Shared] [P] Add the user-template arm to the `renderResult` dispatch — when the selection is a prepared `*render.UserTemplate`, decode the typed `*T` and write `tmpl.Render(v)`, buffer-then-write. — `renderResult` gains a `tmpl *render.UserTemplate` param; the human+template path is unified through a shared `writeHuman` helper (also used by the walking reads) that maps an execution failure to UsageError(2) and a built-in defect to RuntimeError(1); structured arm unchanged. Unit test `TestWriteHuman_UserTemplateExecuteError` pins buffer-then-write + source naming.
...
- [x] **T005** [Shared] Wire the reads to resolve-selection → read+parse-fail-fast → dispatch — widen the seam to `resolveSelection`, add the injected `readTemplateSource` (file via `os.ReadFile` cwd-relative, stdin via the bounded `isTTY`-guarded reader), parse before assembly, and widen the `-o`/`--output` usage string. — every `--output`-capable read seam (me/me roles/actions/projects, roles/role/tree/subroles, domains/domain, policies/policy, projects/project) now declares `resolveSelection` + `readTemplateSource`; the shared `resolveRenderTarget` helper runs resolve→read→parse fail-fast (returning a `renderTarget{format, tmpl}`), `reportTemplateError` maps source failures to UsageError(2), and `root.go`'s `-o` usage string names a template path and `stdin`. New godog suite `user-defined-template-output.feature` (8 scenarios un-@wip'd, 3 @validation held); existing 020/projection suites stay green (a non-token `--output xml` now fails fast as a missing-template-file usage error — same outcome, no request).
