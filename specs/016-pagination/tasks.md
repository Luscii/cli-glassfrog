...
- [ ] **T002** [Shared] Implement the `internal/paging` walker — `All[T]` over an `Executor` seam, with `Result[T]`, `WithPageSize`, and `MalformedPageError` — RED-first unit tests
  ...
  - **Acceptance criteria**:
    ...
    - `HasNextPage=true` + blank/absent `next_cursor` returns `*MalformedPageError` with `Complete:false` and **does not loop** — pinned by a call-counting fake `Executor` tripwire
