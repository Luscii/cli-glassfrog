...
internal/paging               (NEW — the walk; imports glassfrog + apiclient)        [ADR-1]
  ...
  All[T any](reqCtx, ex Executor, req apiclient.Request, opts ...Option) Result[T]   ── the walker  [ADR-1]
    ...
    │    if pg.NextCursor=="" || pg.NextCursor==cursor → return Result{acc, Complete:false, Stop:&MalformedPageError{}} ── non-advancing (blank OR repeated): no loop [ADR-5]
...
**Context**: Spec decision 3 — walk to completion (no mandatory cap), but never loop on a malformed paging response. The cursor must **advance** each page; if it doesn't — `next_cursor` absent, blank, **or identical to the cursor just sent** — the walk must fail loud, not spin. The repeated-cursor case is not hypothetical: an API that ignores an unrecognized `cursor` query parameter (the `cursor`-vs-`after` spec ambiguity in spec § Assumptions) would return the same page with the same non-blank cursor forever, and a blank-only guard would not catch it (risk H-2/H-9). The stop must flow through the consumer's existing `classifyClientError` (§101), which has a `default→1` (RuntimeError) fail-safe and reserved 3–6 for genuine API/network outcomes.

**Options considered**:
1. **Unbounded loop (until `has_next_page=false`) + a non-advancing-cursor guard**: when `has_next_page=true` but the cursor does not advance (`next_cursor==""` **or** `next_cursor==` the cursor just used), stop with a typed `MalformedPageError` (`Complete:false`), never re-issuing. No fixed page/item cap.
2. **Blank-cursor-only guard**: stop only when `next_cursor==""`. Rejected: a repeated non-blank cursor (an API ignoring the param, or a buggy `has_next_page`) loops forever past this guard — the exact H-2/H-9 hazard. The guard must compare against the prior cursor, not just check emptiness.
3. **Impose a hard max-pages cap** that stops loudly when hit. Rejected: the user declined a mandatory ceiling (decision 3); a silent cap would itself be truncation, and a loud one adds a knob nothing yet needs. (A future `WithMaxPages` option remains open without changing the contract.)
4. **No guard, trust the API to terminate**. Rejected: a buggy/adversarial non-advancing response would loop forever — the exact fail-loud-not-spin hazard decision 3 names.

**Decision**: Option 1. The loop runs until a page reports `has_next_page=false` (which also covers an **absent** `meta.pagination` — the zero-valued `HasNextPage` is `false`, so a non-paginated endpoint like the org role tree completes in one page). The walker tracks the cursor it sent and treats a returned `next_cursor` that is blank **or equal to that prior cursor** as non-advancing, yielding `Result{Complete:false, Stop:&MalformedPageError{Page:n}}`. `MalformedPageError` carries no status, so it falls to `classifyClientError`'s `default→RuntimeError(1)` — **no new `Outcome`/`ExitCode` case, no renumbering** (§101).

**Consequences**: Large reads complete; non-paginated endpoints work unchanged; a non-advancing cursor — blank or repeated — fails loud as a partial-incomplete result instead of hanging. Comparing against the prior cursor also caps the worst case of the `cursor`-name mismatch (H-9): if `cursor` is the wrong param and the API ignores it, the walk stops flagged-incomplete rather than looping on the repeated cursor. The default classification means 016 needs zero `internal/cli` edits — the consumer maps `MalformedPageError` via the fail-safe it already has. A bounded variant stays a clean future addition. *Feature-local.*
