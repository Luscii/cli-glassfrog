...
- [x] **T003** [US3] Add the `ci-success` aggregation job and document/script the branch-protection rule — `ci-success` job (needs [lint,test], if: always(), guard asserts both succeeded); additive branch-protection via committed `scripts/setup-branch-protection.sh` (repository ruleset, no replace-on-write); removed @wip from 7 behavioral scenarios (3 @validation held for /score:validate); verified actionlint + shellcheck + JSON-payload + bash -n clean. 3 LEARNINGS logged (toolchain-pin, linter-set tuning, unbound-feature/no-proxy)
  - **Scope**: Add a `ci-success` job (`runs-on: ubuntu-latest`, `needs: [lint, test]`, `if: always()`) whose single guard step exits non-zero unless `needs.lint.result == "success"` and `needs.test.result == "success"` (the `needs.test.result` aggregates the whole matrix, so a failed or cancelled cell fails the gate). Document and provide the maintainer-run branch-protection step that adds `ci-success` as a required check on `main` **additively** — preferably a repository ruleset (`POST …/rulesets`), or, when classic protection already exists, `gh api --method POST repos/{owner}/{repo}/branches/main/protection/required_status_checks/contexts -f 'contexts[]=ci-success'`. Do **not** use the full-document `PUT …/protection` (replaces the whole config, disabling omitted/null fields), and do **not** `PATCH …/required_status_checks` with only `ci-success` (replaces the contexts list, dropping other required checks) — if a PATCH is unavoidable, send the union with the existing contexts. Note that the additive contexts POST 404s when no protection exists yet, so initial setup must create protection (ruleset or UI) first. Capture it in a short `CONTRIBUTING`/CI note or a committed setup script. Only `ci-success` is required (never the drifting matrix-cell contexts).
  - **Acceptance criteria**:
    ...
    - Applying the step adds `ci-success` to the required checks **additively** — it does not drop other already-required status checks, and does not disable or overwrite any pre-existing review requirements or restrictions.
