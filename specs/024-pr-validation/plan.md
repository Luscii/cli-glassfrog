...
**Decision**: Option 1. PR Validation owns: (a) emitting the stable `ci-success` status, and (b) the branch-protection requirement that makes it merge-blocking on `main`, captured as a documented and scripted maintainer step that adds `ci-success` **additively** — a repository ruleset, or (on existing classic protection) the `required_status_checks/contexts` sub-resource — **not** the full-document `PUT …/protection` (replaces the whole config) and **not** a `PATCH …/required_status_checks` sending only `ci-success` (replaces the contexts list, dropping other required checks). Fail-closed follows from GitHub's required-check semantics: a missing or failed `ci-success` blocks the merge. The exact API payload / ruleset JSON is interface-level.
