...
**Observation (not a finding — outside spec-conformance scope):** `.score/memory/LEARNINGS.md` records a design tension where a maintainer hand-applying `no-release-note` to a *noteworthy* PR would have it stripped by 028's sync labeler on the next PR event. This is a spec/plan-level design question about the `no-release-note` "maintainer-flagged" framing (which lives in `settings.yml`'s description and the interface error table, not in spec.md's behavioral guarantees). The implementation faithfully realizes ADR-4's specified exclusion behavior; spec.md § Exclusion only promises that a PR *marked* with the label is omitted, which holds. Worth the authors' attention for a future cycle (candidate for `/score:clarify` if maintainer-flagging of code PRs is intended to be durable), but it does not block this verdict.
