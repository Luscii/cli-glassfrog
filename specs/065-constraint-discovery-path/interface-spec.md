...
| domains | Domain[] | Governing domains in view: `id`, `role_id` (the role that holds it — always `role_…`; a circle is a role with `type: circle`, no `circle_` id namespace), `role_name`, brief, `owned_by_caller` (tri-state — `true`/`false`/**uncertain** — whether `role_id` is among the caller's own roles, from `me roles`; **uncertain** when the `me roles` list is incomplete, since `me roles` does not follow pagination — an unconfirmed match is never reported as a definite `false`) |
| policies | Policy[] | Binding policies in view: `id` (`pol_…`), `role_id`, brief of what it grants/limits |
| characterization | string | A summary of the authority situation **drawn from the record**, **composed** from three independent parts (not a single mutually-exclusive value): (1) the **domain finding** — the governing domain is *held by a role the caller fills* (within their own authority), *held by another role* (names it + id → needs that role's permission or a proposal), or *no domain in the record governs it* (reports the absence plainly — **not** a "you are permitted" verdict); (2) **any policies** that shape or limit the action, surfaced **even when the action is within the caller's own domain** (policies compose with the domain finding, they are never an alternative to it); (3) whether the action is a **change to governance structure** (→ goes through a proposal by default). When the match is ambiguous, the characterization is **the record does not clearly answer** (with what was found). Never an allow/deny verdict computed from local rules |
...
4. The subagent runs the traversal **in its own context**: `search` on the action → resolve matched ids into `roles [id]` / `domains <role_id>` / `policies <role_id>` (or `policy <pol_id>` when search returns a policy directly) → read the caller's own roles (`me roles`) to mark `owned_by_caller` → follow into sub-roles (`tree`) only as far as relevance warrants. Multi-page reads that **support paging** (`search`, `roles`, `domains`, `policies`) are paged through in full (orientation pagination) before the picture is narrowed. `me roles` does **not** follow pagination and can return an incomplete list (it emits an incompleteness note): when it signals incompleteness the navigator marks `owned_by_caller` **uncertain** (never a definite `false`) and records the incompleteness in `notes`, so a possibly-missing own-role is never misattributed as "another role's".
