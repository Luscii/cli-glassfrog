...
- [x] **T001** [Shared] Add the request descriptor and the `Client` built once from the `ConnectionContext`, with the base-URL fail-fast and the `AuthTransport` replay-thunk wiring — RED-first unit tests — 5 unit tests; reused existing `completeContext` helper; no findings
...
- [x] **T002** [Shared] Implement `Execute`: join URL, build the request, make exactly one `Do`, and map the result to a `Response` or a typed `TransportError` / `ResponseError` / `DecodeError` (propagating 007's `AuthError`) — RED-first unit tests — 12 unit tests (every branch + one-attempt tripwire, timeout, token-never-rendered, URL-join); no findings
...
- [x] **T003** [Shared] Make the 010 driving scenarios pass as executable acceptance via godog, driving `NewClient`/`Execute` over a fake base `RoundTripper` in a new suite scoped to this spec's own feature file — 10 behavioral scenarios passing in new `TestRequestExecutionFeatures` (4 `@validation` held `@wip`); all four apiclient suites report own counts (8/10/8/10); reused package fakes; no findings
