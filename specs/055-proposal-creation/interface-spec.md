...
| `newProposalCreateCommand` | `(seam proposalSeam) *cobra.Command` | Guard-registered `create` leaf (`Use:"create <tension-id>"`, `Args: cobra.ExactArgs(1)`, non-empty `Short`, `SilenceErrors`/`SilenceUsage`); declares `--changes`; reads inherited `--base-url`/`--output`; delegates to `runProposalCreate`. |
