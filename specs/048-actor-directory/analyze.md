...
- **K6 — spec User Scenarios → interface coverage (pass)**: All three user scenarios have interface coverage — find whom to contact for a role (`--role-id`), turn a name into an id (`--query`), and tell people apart from agents (`--kind`). Each maps to a documented flag on the `actors` surface.
...
- **Completeness Rule has no Connextra User Scenario** (within-feature note, not a cross-artifact finding): the feature file's "Trust the directory is whole" Rule block is derived from the spec's "Completeness of the list" behavioral accord rather than a spec User Scenario (the spec has 3 User Scenarios, none for completeness — unlike sibling 038, which had a fourth). K1 (driving scenarios → feature) and K6 (user scenarios → interface) both pass because the completeness *driving* scenario (first-page opt-out) is present and all 3 *user* scenarios are covered. A `/score:clarify` pass could add a fourth completeness user scenario for parity with 038 — optional, non-blocking. Surfaced here for transparency; it is not an analyze check failure.
