...
> **Cross-spec dependency — read before starting Phase 2.** 020 is the selector/router that consumes two seams built by other specs: `internal/output` (018 Structured Serialization — `Format`, `RenderSuccess`) and `internal/render` (019 Templated Human Rendering — `Render`, `FormatFull`/`FormatCompact`, the `Resource` keys). On this branch **neither is merged** (the four reads still carry inline `formatXxx`). Phase 1 (the `internal/output` selection vocabulary + resolver) depends on neither and can proceed now. Phase 2's dispatch (T005/T006) is **gated on both 018 and 019 landing on main**, and T006 rewires the *post-019* read commands (which by then call `render.Render`, not `formatXxx`). Sequence Phase 2 after both merge. (plan Risks; DECISIONS 2026-06-08.)
>
...
- [ ] **T005** [Shared] [P] Add the shared generic render-dispatch in `internal/cli`
  ...
  - **Risk**: ⚠️ Cross-spec — consumes 018 + 019 seams that are not yet on main; cannot land until both merge.
