...
**Options considered**:
1. **Best-effort `internal/build` drift test** — assert every command leaf the skill/agent name (`search`, `roles get`, `tree`, `roles fillers`, `subrole-actors`, `roles domains`, `roles policies`) exists in the CLI's command registry. Pins the enumerable surface; leaves prose to review. Reuses the established home and idiom.
...
### ADR-5: Read-only tool grant enforces "must not write"; "surfacing, not judging" is prompt-level, not tool-enforceable

**Context**: The spec's non-behaviors require the path never to write (deferring capture to 066 and drafting to the proposal paths) and never to judge authority (deferring that to Constraint Discovery 065). ADR-1's subagent form lets us realize the first structurally.

**Options considered**:
1. **Restrict the subagent's tools to read commands only** — grant the navigator only the read paths (search/role reads/fillers/domains/policies), withholding any tool that could run a `glassfrog` proposal/tension write. Makes "must not write" impossible by construction.
2. **Rely on prompt instruction alone** — tell the agent not to write. Weaker: an agent with write tools available can still be steered into a write.

**Decision**: Option 1 for writes — the navigator's tool grant excludes governance-write commands, so "must not write" is structural. The **boundary with 065** ("surface domains/policies, do not judge authority") *cannot* be tool-enforced — reading a policy is legitimate for both paths — so it is enforced at the prompt level: the skill/agent describe surfacing the governing governance and explicitly hand any "am I allowed to do X?" judgment to the Constraint Discovery Path (065).

**Consequences**: The most dangerous non-behavior (writing) is structurally impossible for the navigator; the softer boundary (judging) is a prose guardrail the artifacts state plainly. If the host cannot honour a read-only tool grant, the write-safety guardrail (063) still stands between any write and the record, so defense is layered, not single-point. The exact tool-grant syntax is interface-level.
