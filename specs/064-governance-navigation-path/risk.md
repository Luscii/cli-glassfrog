...
| H-4 | Host does not honor the read-only tool grant → the navigator issues a governance write from a "read" path | interface § Error Communication (governance-write nuance); plan ADR-5, R2 | High | Low | Yellow | RC-8, RC-9, RC-10 | **Yellow** |
...
**H-4 — Host ignores the read-only tool grant.** *Severity High* (a governance write from a read path violates IX), *Probability Low*. Controls: **RC-8** the agent's `tools` grant withholds Write/Edit (no local mutation); **RC-9** the agent prompt scopes execution to the read leaves; **RC-10** 063's `PreToolUse` write hook gates any `glassfrog` proposal-write Bash call regardless (layered defense). Residual **Yellow** — the guarantee leans on an external host honoring the grant *and* on 063 being installed (064 is a read path and does not itself depend on 063), so the backstop is not certain when 064 ships ahead of 063. An external-contract residual, like 063's own host-contract hazards.
...
| RC-10 | 063 `PreToolUse` write hook gates any proposal-write Bash call (layered) | H-4 |
