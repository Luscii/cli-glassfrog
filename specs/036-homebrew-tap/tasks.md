...
- [ ] **T003** [Shared] Add the `brews` block to `.goreleaser.yaml` with the 022-config refinements, a config-guard assertion, and an offline render-and-inspect test
  ...
  - **Acceptance criteria**:
    - `goreleaser release --snapshot --clean --skip=publish` renders a `Formula/glassfrog.rb` with all four platform `url`+`sha256` blocks, the install and test stanzas, and `version` from the tag.
