...
- [x] **T001** [Shared] [P] Provision the dedicated tap repository and its cross-repo write token — created `Luscii/homebrew-cli-glassfrog` (internal for now → public later; default branch `main`, unprotected). Credential: the org secret `CI_GITHUB_TOKEN` is injected as `HOMEBREW_TAP_TOKEN` in the tap job (no new secret minted; org-wide scope vs ADR-4's tap-only — recorded in DEPRECATION.md). Verify `CI_GITHUB_TOKEN` has `contents: write` on the tap repo before the first stable release.
...
- [x] **T002** [Shared] [P] Decide and add the project LICENSE, then wire the formula's `license` metadata — added an **MIT** `LICENSE` (Luscii, 2026) at the repo root and set `brews.license: "MIT"` explicitly (GoReleaser does not populate it from the LICENSE file during an offline render). Rendered formula now carries `license "MIT"`, so `brew audit --strict` no longer fails on a missing license.
...
- [x] **T004** [Shared] Add the post-publish `tap` job to `release.yml` (brew-publisher-only, stable-only) — added the `tap` job (`needs: [publish]`, `if: !github.event.release.prerelease`, `HOMEBREW_TAP_TOKEN` ← `secrets.CI_GITHUB_TOKEN`, brew-publisher-only `goreleaser release` with `release.disable: true` keeping the GitHub release untouched); pinned GoReleaser to `~> v2.16` (keep `brews` despite its deprecation — see DEPRECATION.md). Extended the `internal/build` workflow guard (`CheckTapJob`) with change-detector coverage of all of the above + drift tests. Live `brew install` from the tap remains a manual post-first-release check (and needs the repo flipped to public); the held `@wip` install/upgrade/integrity scenarios stay for that + `/score:validate`.
