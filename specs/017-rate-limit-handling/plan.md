...
**Consequences**: 017 stays a clean transport-layer behavior; `internal/apiclient` does not import `internal/cli`. A surfaced (capped-out) 429 exits 3 today — defensible, since the surfaced case is the rare exhaustion path and most 429s resolve transparently. *Precedent-setting: a 429 surfaced after retry exhaustion is a generic `APIError(3)` until 015 types it; rate-limit *handling* (retry) and rate-limit *classification* (exit code 5) are separate capabilities.*
...
- **Surfaced 429 exits 3, not the reserved 5** (medium likelihood of reviewer/operator surprise, low impact): a capped-out 429 maps to `APIError(3)` until 015 types it. *Mitigation*: ADR-5 makes this an explicit, spec-faithful boundary; documented in the handoff and "Does Not Cover" so 015 picks it up; the retry value (transparent recovery) is unaffected.
