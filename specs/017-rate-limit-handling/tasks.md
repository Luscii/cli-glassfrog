...
- [ ] **T001** [Shared] Add the `RetryPolicy` value (+ `DefaultRetryPolicy` constants), the `isSafeMethod` predicate, and the `parseRetryAfter` header parser — RED-first unit tests
...
- [ ] **T002** [Shared] Implement `RetryExecutor` + `NewRetryExecutor`: wrap `(*Client).Execute` in a bounded `429` retry loop with injected `sleep` + progress seams, surfacing the raw 429 at the caps — RED-first unit tests over a fake base + recording sleep + buffer
