...
- [x] **T003** [P] [Shared] Route 011's `runMe` send through the retry executor, threading `time.Sleep` and the command's stderr in production — RED-first unit tests over a fake transport + fake sleep — `me.go` (seam `sleep()` method, executor at the send site); new 429-then-200 `me` test, existing branches unchanged
  ...
  - **Acceptance criteria**:
    ...
    - A `me` run with a non-429 outcome (200 success, transport error, no-token) behaves exactly as before — 017 makes no `classifyClientError` edit; the shared classifier owns each outcome's mapping (a generic non-2xx → `APIError(3)`; with 015 landed, 401/403 → `PermissionError(4)` and a capped-out 429 → `RateLimited(5)`)
...
- [x] **T004** [P] [Shared] Make the 017 driving scenarios pass as executable acceptance via godog, driving `NewRetryExecutor`/`Execute` over a fake base + recording sleep in a new suite scoped to this spec's own feature file — `rate_limit_handling_bdd_test.go` (5th apiclient suite, its own feature file); 8 behavioral scenarios un-wipped + passing, 4 `@validation` kept `@wip`; all five suites report independent counts
