...
- [x] **T001** [US2] Grow the shared `internal/glassfrog.Role` and add the `/me/roles` response type + pure renderer — schema growth (Role + Pagination + MyRolesResponse, snake_case tags), pure `formatMyRoles`/`incomplete`; decode + renderer unit tests pass, no findings
