...
- **Token leak through output or an error line** (low likelihood, high impact): first command class on the live request path. *Mitigation*: never reads the token; token-never-in-output test across all branches; never a response-model field.
