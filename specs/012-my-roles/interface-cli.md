...
| API returned a non-2xx response (generic; incl. 401/403/429) | `*ResponseError` | `APIError` | 3 | names the HTTP status and reports the read failed, plus a **generic** next step ("the API rejected the read; check that the token has access and retry, or consult the status code"). It does **not** interpret the status into a specific meaning (no "403 → permission" / "429 → rate-limited") — that classification is API Error Extraction's (015) job (spec Non-Behavior). |
