...
- [x] **T003** [Shared] Wire `my projects` under the `my` parent in `Assemble()`, and make the driving scenarios pass as executable acceptance via a new `internal/cli` godog suite — wired `me projects` under `me` (one MustRegister line in app.go); new `TestMyProjectsFeatures` godog suite (Paths → only my-projects.feature); 12 behavioral scenarios un-@wip'd and passing, 5 @validation scenarios kept @wip; reused shared step phrasings
