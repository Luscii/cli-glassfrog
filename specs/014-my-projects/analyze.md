...
- **K5** interface-cli.md § Surface → my-projects.feature — the command, `--status` flag (supported + unsupported paths), the projection (incl. empty-list and no-role), the "more available" signal, the `--base-url` error path, and the **malformed credential file (`CredentialError`→exit 1)** path all have scenario coverage. _Correction (PR review): the initial K5 pass overstated coverage — the `CredentialError` branch in the interface-cli error table had no Gherkin scenario. Closed by adding "A malformed credentials file fails the read loudly" to my-projects.feature (mirroring 011's guard-derived scenario). K5 now genuinely holds._
