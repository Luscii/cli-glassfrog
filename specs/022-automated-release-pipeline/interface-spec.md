...
| Job `publish` | `needs: [build, verify]` (runs only if build and **every** verify leg succeed). Downloads `dist/` and uploads **only the release assets** to the triggering release: `gh release upload "${{ github.event.release.tag_name }}" dist/*.tar.gz dist/*checksums.txt --clobber`. The glob must exclude GoReleaser's `dist/` metadata (`artifacts.json`, `metadata.json`, `config.yaml`) and per-target build subdirectories — uploading those would attach junk assets or fail on a directory. Robust alternative: derive the exact asset paths from `dist/artifacts.json` (filter `type` ∈ {`Archive`, `Checksum`}) rather than globbing. |
