...
- [ ] **T002** [Shared] Replace the stub `roles` group with the command scaffold + the bare org-wide list (walk to completion) + render + wiring — RED-first unit tests for every branch; replaces existing `internal/cli/roles.go` + `roles_test.go`, new `org-roles` render templates
