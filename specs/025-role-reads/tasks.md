...
>
> **All cross-spec dependencies are landed on main.** Per STATUS.md, 007/009/010/015/016/017/018/020 are Complete and 019 is at "Ready" — its `internal/render` package is landed on main (the `roles.full/compact.tmpl` templates `me roles` uses ship from it). So the 025 base is cut from current main with no sequencing caveat. **Existing main state 025 builds on, not around**: `internal/cli/roles.go` already exists as a wired (`Assemble()`) `roles` *group* with `list`/`get` "not yet implemented" stubs, and `render.ResourceRoles` (`"roles"`) is already used by `glassfrog me roles` (`me_roles.go`). 025 **replaces** the stub group with its real command and adds **new** render keys (`org-roles`, `role`) — it does not reuse or clobber `roles`. T001 reuses `internal/glassfrog` (011/012) + `Page[T]`/`Pagination` (016); T002–T004 reuse `AssembleFromOS`/`NewClientFromOS`/`Execute` (009/010), `NewRetryExecutor` (017), `paging.All` (016), `classifyClientError`/`Outcome`/`ExitCode` (011/015), the persistent `--base-url` (011) + `--output`/`-o` (020), and `renderResult[T]` over `internal/output`/`internal/render` (018/019/020). 025 adds **no** new `Outcome` category, `ExitCode` case, validator-beyond-its-own, or root flag.
>
...
- [x] **T001** [Shared] [P] Grow `Role` to the full spec shape and add `RoleDetail` + the `Assignment`/`Policy`/`Note`/`SkillSummary` leaf models and the `{data: RoleDetail}` document wrapper — RED-first decode tests; `internal/glassfrog/roles.go` (+ related model files) + tests — 5 decode tests (Page[Role] grown fields, RoleDocument detail + omitted includes); moved Role/Accountability/Domain into roles.go
...
- [x] **T002** [Shared] Replace the stub `roles` group with the command scaffold + the bare org-wide list (walk to completion) + render + wiring — RED-first unit tests for every branch; replaces existing `internal/cli/roles.go` + `roles_test.go`, new `org-roles` render templates — list walk + structured + org-roles full/compact + validateRolesFlags + Assemble wiring; smoke "bare group help" case moved to `auth`
  - **Scope**: **Replace** the existing stub `internal/cli/roles.go` (a wired `roles` group with `list`/`get` "not yet implemented" stubs) with the real command. `newRolesCommand(seam rolesSeam) *cobra.Command`: a guard-ready leaf (`Use:"roles [id]"`, `Args: cobra.MaximumNArgs(1)`, non-empty `Short`, `SilenceErrors`/`SilenceUsage`), reads the persistent `--base-url` (011) + `--output`/`-o` (020), delegates to pure `runRoles`. `runRoles(cfg) (Outcome, error)`: resolve `--output` (020) and run `validateRolesFlags` (id/flag-combo guard) **before** assembly; with no id → `runRolesList`. `runRolesList`: default path walks `GET /roles` to completion via `paging.All[Role]` over the seam's executor (`RetryExecutor` wrapping `NewClientFromOS`, 016/017), then dispatches the records through `renderResult("org-roles", format, records)` (020). On a `Result.Stop` error, map via `classifyClientError`. Register **new** `org-roles` `full`/`compact` templates in `internal/render` (019) — distinct from the shipped `roles` key/templates `me roles` uses (left untouched) — one block per role (name, id, purpose, domains, accountabilities) with explicit-absence markers; registry exhaustiveness guard. **Update** the existing `MustRegister(root, newRolesCommand(...))` wiring in `Assemble()` to the seam-taking constructor (the stub `list`/`get` subcommands are removed). `roles` never reads `ctx.Cred.Token`.
  - **Acceptance criteria**:
    ...
    - The new `org-roles` render key has both `full` and `compact` (the shipped `roles` key/templates for `me roles` are unchanged); the registry guard passes; no `Outcome`/`ExitCode`/root-flag is added
...
- [x] **T003** [Shared] Add the list filters, the `--first-page` opt-out, the completeness signalling, and `--per-page` — RED-first unit tests; extends `internal/cli/roles.go` — parent/person/tag/tri-state has-subroles passthrough, --first-page single-page+signal, mid-walk partial+non-zero, --per-page WithPageSize
...
- [x] **T005** [Shared] Make the driving scenarios pass as executable acceptance — new `internal/cli` godog suite over `role-reads.feature`; un-@wip the behavioral scenarios, keep `@validation` held — TestRoleReadsFeatures (Paths → role-reads.feature only): 13 behavioral scenarios pass, 3 @validation kept @wip; fake base transport (single/multi-page/mid-walk)
