...
| `runRoleGet` | `(cfg, exec, format, id string) (Outcome, error)` | One `Execute` into a `RoleDocument` (`{data: RoleDetail}`); `?include=` from the validated `--include` (comma-joined); the id is escaped as a single path segment (`url.PathEscape`) but passed through unvalidated (ADR-4). Structured `--output` emits the raw payload verbatim; the human path renders the `role` template over a `render.RoleView`. Writes to `cfg.stdout`; returns the code-free `Outcome`. |
