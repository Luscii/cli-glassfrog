...
- **V Composition over Monolith** → "The feature replaces the stub `roles.go` with its real command + adds two new render keys (`org-roles`/`role`, distinct from the shipped `roles`) without editing sibling command modules; the existing `roles` wiring line is updated in place; reused symbols are declared, not hidden."
...
→ **plan.md System Architecture + tasks.md T002**: 025 replaces the existing stub `internal/cli/roles.go` (a `roles` group with `list`/`get` placeholders) with the real command, grows the shared `glassfrog.Role`, adds two new `internal/render` keys (`org-roles`/`role`, distinct from the shipped `roles`), and updates the existing `MustRegister(root, newRolesCommand(...))` wiring in `Assemble()`. No edit to sibling command modules; the pure `runRoles`/`runRolesList`/`runRoleGet` are independently unit-testable over the injected seam. PASS.
