...
**Data flow**: `actors <id> [--include]` → `resolveFormat(--output)` (020) → mode-separation guards (filters list-only, `--include` single-only) → `validateActorInclude(--include)` → `AssembleFromOS(--base-url)` (009) → `NewClientFromOS` (010) → `Execute(GET /actors/{id}?include=…)` into `{data: ActorDetail}` (010) → `Result[ActorDetail]` → single-resource render via `renderResult` (for `json`/`yaml`, the 2xx body captured as `json.RawMessage` → `output.RenderSuccess` — the single-resource raw-bytes path, **not** the walked-list `aggregateRawData`; for `full`/`compact`, the `actor` projection) → stdout. Failures route through the shared `classifyClientError` chain (011/015) → 004 exit code. A `404` for an unknown id is a clean non-2xx (no local id validation — 025 ADR-4).
...
**Input validation order**: `--output` resolves first (020), then the mode-separation guards (list-only filters / single-only `--include`, ADR-1), then `--include` is validated (pure, no I/O) — all ahead of any context assembly, so a transport tripwire can assert no request issued on rejection (the 011/048 fail-fast discipline). This `--output` → mode-separation → `--include` order matches the interface contract and tasks T003, pinning which error surfaces first when several inputs are invalid.
