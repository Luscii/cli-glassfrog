...
**Dispatch**: the command's single `RunE` branches on `len(args)`. For the single read (1 arg), before any network call, in order: (1) cobra `Args`/flag parsing (a third positional fails as a `MaximumNArgs(1)` violation; an unknown flag fails here); (2) mode-separation guards (a list filter with an id, `--include` without an id) → usage error; (3) `--output` resolution (020); (4) `--include` validation (the reject-unknown validator over `{roles, assignments}`). Output resolution precedes include validation so an invalid `--output` is reported even when `--include` is also invalid — both are pure, pre-assembly checks, so either order keeps the no-request guarantee. Any failure here is a fail-fast usage error and **no request is sent** (a transport tripwire asserts this, per 011/025/048).
