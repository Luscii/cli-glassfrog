...
- [ ] **T001** [Shared] Add the `ActorDetail` decode type embedding `Actor` + optional `Roles`/`Assignments` — unit-test the `{data: ActorDetail}` decode; `internal/glassfrog` (new `ActorDetail`) + decode tests
...
- [ ] **T002** [Shared] Add the singular `actor` render key + view + templates — golden + registry-guard tests; `internal/render` (new `actor.{full,compact}.tmpl`, `ActorDetailView`, `ResourceActor`) + tests
...
- [ ] **T003** [Shared] Grow `actors` to `MaximumNArgs(1)` + the single-read branch + `--include` validator + mode-separation guards — RED-first unit tests for every branch; grows `internal/cli/actors.go` (+ the `--include` validator)
...
- [ ] **T004** [Shared] Make the driving scenarios pass as executable acceptance — new/extended `internal/cli` godog suite over `actor-read.feature`; un-`@wip` the behavioral scenarios, keep `@validation` held
