...
- [x] **T001** [Shared] Add the `ConnectionContext` value object pairing 008's `BaseURL` outcome with 005's `auth.Resolution` outcome, with derived readiness and a redacting `String()` — RED-first unit tests — 3 test groups (Complete/Problems/redaction), no findings
...
- [x] **T002** [Shared] Implement the transparent aggregator: call both resolvers once, carry both outcomes, always return a context — plus the production seam — RED-first unit tests — outcome-combination table + carry-both tripwire + determinism + nil-panic + FromOS binding, no findings
...
- [x] **T003** [Shared] Make the 009 driving scenarios pass as executable acceptance via godog, driving `Assemble` over fake resolver outcomes in a new suite scoped to this spec's own feature file — 8 behavioral scenarios un-wipped & passing, 4 @validation kept @wip; new `TestConnectionContextFeatures` suite; all three apiclient suites report independent counts (10/8/8), no findings
