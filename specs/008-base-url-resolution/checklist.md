...
- **P0** | CONSTITUTION I (Spec Fidelity): built-in default base URL traces to the spec — **pass**. plan.md § Integration Design and interface-spec.md (default row) pin `https://glassfrog.com/api/v5` from `spec/glassfrog-api-v5.yaml` (servers `url: /api/v5` resolved against `info.contact.url`). The slice invents no endpoints/parameters (resolution only). *Nuance:* the host is derived (relative server URL), flagged in plan Risks and worth an analyze/risk look — not a violation.
