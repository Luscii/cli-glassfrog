...
- **Implementation location diverged from the task/plan text — deliberately.** T001, plan ADR-3, and interface-spec.md describe the `base_url` file read living in `internal/auth` (beside the token reader). A post-implementation refactor moved the generic `.glassfrogrc` read/parse/walk into a new `internal/rcfile` package, with `auth` and `apiclient` as consumers and the `base_url` key owned by `apiclient`. Behavior is unchanged and fully conformant. The move is recorded in `.score/memory/DECISIONS.md` as an explicit supersession of 005 ADR-1 / 008 ADR-3. The 008 spec artifacts (plan ADR-3, tasks.md notes, the interface "shared reader" wording) still describe the `auth` placement and now lag the code — a doc-vs-code drift on the artifacts, not a behavioral gap. Reconciling those artifacts (or leaving them as the historical record beside the DECISIONS supersession) is the developer's call; cross-artifact consistency is analyze's domain, not validate's.
- **Two stale code comments** (code-review scope, outside validate's behavioral lane): `baseurl.go:14` still says the `base_url` key "lives in internal/auth beside the token key" (it now lives in `apiclient`), and `baseurl.go:147` says a broken file "surfaces internal/auth's typed read/format error" (it now surfaces `rcfile`'s). Comment accuracy only — no behavioral effect.
