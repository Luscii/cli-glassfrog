...
- **Glassfrog API v5 specification (reference)**: the built-in default base URL takes its `/api/v5` path from `spec/glassfrog-api-v5.yaml`'s `servers:` block; the host is inferred from `info.contact.url` (an assumption — see the Built-in default value assumption below).
...
- **Built-in default value** `[ASSUMED]`: the default base URL's `/api/v5` path comes from `spec/glassfrog-api-v5.yaml`'s `servers:` block, but that server URL is relative and states no host; the host is inferred from `info.contact.url` (not normatively specified by OpenAPI). The concrete string is fixed during planning. (Not yet hardcoded anywhere in the codebase.)
