...
- *Config-guard*: the existing `internal/build` `TestConfigGuard_RealConfig` continues to pass after the ldflags edit (the matrix guard ignores `ldflags`); a focused assertion that the real config's `ldflags` injects a v-prefixed value into `internal/cli.version` keeps the seam from silently regressing to empty, a stale target, a malformed `-X`, or a non-v-prefixed value.
...
- **Silent ldflags regression** (low likelihood, medium impact): a future edit to `.goreleaser.yaml` could blank the `ldflags` entry (back to 021's empty seam), and the matrix config-guard would not catch it (it ignores `ldflags`). Releases would then fall to build-info/placeholder and report a wrong version. Mitigation: a focused config assertion that the real config injects a v-prefixed value into `internal/cli.version` (noted in testing strategy).
- **`-X` symbol path or value-shape drift** (low likelihood, medium impact): renaming `internal/cli.version` without updating the ldflags target makes injection a silent no-op (Go ignores `-X` for an unknown symbol), and dropping the `v` prefix violates the release/source-install shape contract. Mitigation: the var's doc comment names the `-X` path; the focused config assertion above catches a stale symbol, malformed `-X`, and a missing `v` prefix, with the release-shape build test covering the produced binary.
