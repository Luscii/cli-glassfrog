...
- [x] **T001** [Shared] Resolve the reported version with a pure three-tier function, re-point the 003 wiring, default the injected var to empty — 5 driving scenarios un-wipped + 8-case resolver unit table; migrated TestVersionDefaultPlaceholder; noted a @validation mis-tag in LEARNINGS.md
...
- [x] **T002** [US3] Fill the `.goreleaser.yaml` `builds.ldflags` injection seam and guard it against regression — 3 scenarios un-wipped; `CheckVersionInjection` guard + drift table in internal/build (blanked seam, stale symbol, malformed `-X`, missing `v` prefix); verified `goreleaser build --snapshot` reports `v0.0.0-SNAPSHOT-<commit>` (v-prefixed, not the placeholder)
  - **Scope**: Set `.goreleaser.yaml` `builds[0].ldflags` to `["-X github.com/Luscii/cli-glassfrog/internal/cli.version=v{{ .Version }}"]` — the `v{{ .Version }}` token restores the `vX.Y.Z` shape (GoReleaser strips the `v` from `.Version`) and keeps snapshots honest (a `v`-prefixed snapshot version, not a stale `.Tag`). Do **not** touch the `builds` matrix, `env` (`CGO_ENABLED=0`), `flags` (`-trimpath`), or 022's `archives`/`checksum`/`release` sections. Adding `-s -w` is not required and out of this seam's contract. Add a focused config-regression test in `internal/build` (alongside the config-guard) asserting the **real** config's `builds[0].ldflags` injects a v-prefixed value into `internal/cli.version` — the matrix config-guard ignores `ldflags`, so this is the guard against a blanked seam, a stale `-X` symbol path, or a dropped `v` prefix.
  - **Acceptance criteria**:
    ...
    - The new config-regression test fails loudly if `builds[0].ldflags` is blanked, no longer injects `internal/cli.version`, or drops the required `v` prefix.
