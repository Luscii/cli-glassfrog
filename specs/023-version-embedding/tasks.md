...
- [x] **T002** [US3] Fill the `.goreleaser.yaml` `builds.ldflags` injection seam and guard it against regression — 3 scenarios un-wipped; `CheckVersionInjection` guard + drift table in internal/build; verified `goreleaser build --snapshot` reports `v0.0.0-SNAPSHOT-<commit>` (v-prefixed, not the placeholder)
