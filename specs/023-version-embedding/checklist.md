...
- **P0** | III. Fail Safe, Not Silent ("errors MUST be obvious … never hidden"): the silent-wrong-version hazards — a blanked or stale `ldflags` injection seam, or a dropped `v` prefix, reporting a wrong version shape — are caught loudly by a dedicated config-regression guard, not shipped silently (tasks.md T002 acceptance; interface-spec.md § Error Communication; plan.md § Risks). The resolver itself never throws and never returns empty. **Pass.**
