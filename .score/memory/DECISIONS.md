...
- Credential Discovery returns a code-free `Resolution{Token, Source}`; absence is a normal outcome, broken files are errors (from 005-credential-discovery, 2026-06-03)
  Mirrors 002's "resolver classifies, consumer maps" split. `Source ∈ {Environment, File(path), None}`; `None` is a non-error "nothing found"; a read/parse failure of an existing file is a typed error naming the path. The token value never appears in output, logs, or error strings (secret hygiene; CONSTITUTION II). 007 consumes this shape and reports `Source` (never the token) as the active identity. No exit codes here.
