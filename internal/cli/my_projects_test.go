...
// header, asserted absent from output by runMyProjectsOver).
const (
	projectsBodyMulti = `{
      "data": [
...
// runMyProjectsOver drives the pure runMyProjects over a fake seam, returning the
// outcome and the captured stdout/stderr, and failing if the token leaks.
func runMyProjectsOver(t *testing.T, seam meSeam, status string) (Outcome, string, string) {
	t.Helper()
	var out, errb bytes.Buffer
	outcome, _ := runMyProjects(myProjectsConfig{
		seam:   seam,
		status: status,
...
// --- formatMyProjects (pure) ---------------------------------------------

func TestFormatMyProjects_RendersFieldsPerProject(t *testing.T) {
...
func TestFormatMyProjects_NullRoleRendersNoRoleMarker(t *testing.T) {
	resp := glassfrog.MyProjectsResponse{
		Data: []glassfrog.Project{
...
func TestFormatMyProjects_EmptyListRendersExplicitLine(t *testing.T) {
	out := formatMyProjects(glassfrog.MyProjectsResponse{})
...
func TestRunMyProjects_NullRoleRendersNoRoleMarker(t *testing.T) {
	tr := &cannedTransport{status: 200, body: projectsBodyNoRole}
	seam := &fakeMeSeam{ctx: validMeContext(), transport: tr}

	outcome, stdout, _ := runMyProjectsOver(t, seam, "")
	if outcome != Success {
		t.Fatalf("outcome = %v, want Success", outcome)
	}
...
func TestRunMyProjects_EmptyListIsCleanSuccess(t *testing.T) {
...
func TestRunMyProjects_SupportedStatusFiltersRequest(t *testing.T) {
	tr := &cannedTransport{status: 200, body: projectsBodyMulti}
	seam := &fakeMeSeam{ctx: validMeContext(), transport: tr}

	outcome, _, _ := runMyProjectsOver(t, seam, "current")
	if outcome != Success {
		t.Fatalf("outcome = %v, want Success", outcome)
	}
	if tr.calls != 1 {
...
func TestRunMyProjects_UnsupportedStatusRejectedBeforeAnyRequest(t *testing.T) {
	tr := &cannedTransport{status: 200, body: projectsBodyMulti}
	seam := &fakeMeSeam{ctx: validMeContext(), transport: tr}

	outcome, _, stderr := runMyProjectsOver(t, seam, "in-progress")
...
func TestRunMyProjects_HasNextPageSignalsMoreOnStderr(t *testing.T) {
...
func TestRunMyProjects_NoCredentialsIsUsageErrorNoRequest(t *testing.T) {
	tr := &cannedTransport{status: 200, body: projectsBodyMulti}
...
func TestRunMyProjects_CredentialErrorIsRuntimeError(t *testing.T) {
	tr := &cannedTransport{status: 200, body: projectsBodyMulti}
	ctx := apiclient.ConnectionContext{
		BaseURL: apiclient.BaseURL{Value: "https://example.test/api/v5", Source: apiclient.SourceFlag},
		Cred:    auth.Resolution{Source: auth.SourceNone},
	...
	outcome, _, stderr := runMyProjectsOver(t, seam, "")
...
func TestRunMyProjects_TransportFailureIsNetworkUnavailableNoRetry(t *testing.T) {
...
func TestRunMyProjects_NonStatus2xxIsAPIError(t *testing.T) {
...
func TestRunMyProjects_UndecodableBodyIsRuntimeError(t *testing.T) {
...
func TestRunMyProjects_BaseURLErrorIsUsageErrorNothingSent(t *testing.T) {
	tr := &cannedTransport{status: 200, body: projectsBodyMulti}
	seam := &fakeMeSeam{
		ctx:          apiclient.ConnectionContext{},
...
// --- newMyProjectsCommand integration (outcome → exit code, registration) -

// runMyProjectsCommand registers `me` under a real root (with the persistent
...
func runMyProjectsCommand(t *testing.T, seam meSeam, args ...string) (Outcome, int, string, string) {
	t.Helper()
	root := NewRootCommand()
...
func TestMyProjectsCommand_ExitCodesAcrossOutcomes(t *testing.T) {
	cases := []struct {
		name     string
		tr       *cannedTransport
		ctx      apiclient.ConnectionContext
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			outcome, code, stdout, stderr := runMyProjectsCommand(t, seam, tc.args...)
...
func TestMyProjectsCommand_StrayArgSendsNothing(t *testing.T) {
	tr := &cannedTransport{status: 200, body: projectsBodyMulti}
	seam := &fakeMeSeam{ctx: validMeContext(), transport: tr}
	outcome, code, _, _ := runMyProjectsCommand(t, seam, "extra-argument")
...
func TestMyProjectsCommand_InheritsBaseURLFlag(t *testing.T) {
	tr := &cannedTransport{status: 200, body: projectsBodyMulti}
	seam := &fakeMeSeam{ctx: validMeContext(), transport: tr}
	_, _, _, _ = runMyProjectsCommand(t, seam, "--base-url", "https://flag.test/api/v5")
...
func TestMyProjectsCommand_DeclaresNoOwnBaseURLFlag(t *testing.T) {
	cmd := newMyProjectsCommand(&fakeMeSeam{})
...
func TestMyProjectsCommand_DeclaresLocalStatusFlag(t *testing.T) {
	cmd := newMyProjectsCommand(&fakeMeSeam{})
...
func TestMyProjectsCommand_DeclaresNoIncludeFlag(t *testing.T) {
	cmd := newMyProjectsCommand(&fakeMeSeam{})
...
func TestRunMyProjects_NeverSendsInclude(t *testing.T) {
	tr := &cannedTransport{status: 200, body: projectsBodyMulti}
	seam := &fakeMeSeam{ctx: validMeContext(), transport: tr}
	_, _, _ = runMyProjectsOver(t, seam, "current")
...
func TestMyProjectsCommand_MeIsRunnableWithProjectsChild(t *testing.T) {
	root := NewRootCommand()
	meCmd := newMeCommand(&fakeMeSeam{})
	MustRegister(root, meCmd)
	MustRegister(meCmd, newMyProjectsCommand(&fakeMeSeam{}))
