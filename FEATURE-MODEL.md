...
**The reads it reaches are observed, not inferred.** All six were probed directly during specification (LEARNINGS 2026-08-08, W1–W5): the role list and single-role read, the role tree, the actor list and single-actor read, and the identity read. The tree read is the one that could only be settled by observation — its response schema declares no `legacy_id` at all, yet it returns the number on every node to full depth, so the schema is upstream-defective and a scope decision taken from the contract alone would have dropped a working read. Reading which reads carry the number off the parameter's `$ref` sites is therefore not sufficient; the same caution applies to embedded resources, whose behaviour differs per read rather than following a global rule (W2).
