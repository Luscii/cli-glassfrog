...
The knowledge answer to the problem, served by the CLI itself: the verified grammar rides in the binary every consumer already has, rendered by a dedicated informational read command rather than shipped as an operating-surface file. The command **informs and never validates** — it makes no API request and rejects nothing locally; the server stays the sole judge of what is valid, which keeps VISION Exclusion 2 ("no local governance logic") intact. Contract-published shapes are derived from the vendored v5 spec at build and drift-guarded against it; the empirical residue comes from the recorded grammar facts; every rendered shape carries its provenance marking (contract vs observation). The Agent Operating Surface's Proposal Drafting Path consumes it by running the command — a reference that resolves on any machine with the CLI, plugin or not.

- Agent-Facing Grammar Reference — a dedicated CLI read command that renders the change-set grammar an assembler consults before building: the part shapes for the change types the write path assembles — contract-derived shapes drift-guarded against the vendored spec, and the recorded empirical residue with the symptom each wrong shape produces — every shape marked by its provenance; output informs and never validates
  + depends-on: Change-Set Grammar Facts
- Pre-Assembly Grammar Consultation — the drafting path runs the grammar command before assembling, and surfaces a recognized dead shape before the write rather than after it, so the grammar is applied rather than merely shipped
  + depends-on: Agent-Facing Grammar Reference
  + depends-on: Proposal Drafting Path
...
The API grew the answer: `?include_legacy_id=true` adds an integer `legacy_id` to each resource, verified live to be the same number the web-UI URL shows and the change payloads carry (`.score/memory/LEARNINGS.md`, 2026-08-05, S1/S4). It also resolves both matching limits the harvest route could not — it distinguishes same-named roles, and it labels elements no proposal ever named. **Carries a retirement clock**: the parameter is a declared v3→v5 transition bridge that retires with the v3 API and is documented as something not to build durable integrations on (S3). So it is the right answer *now*, and the surface consuming it should degrade to asking rather than assume it persists.

**The reads it reaches are observed, not inferred.** Every one of them was probed directly while 075 was being specified — the role list, the single-role read, the role tree, the actor list, the single-actor read, and the identity read — and the findings are recorded with that specification. The tree read is the one that could only be settled by observation: its response schema declares no `legacy_id` at all, yet it returns the number on every node to full depth, so the schema is upstream-defective and a scope decision taken from the contract alone would have dropped a working read. Enumerating the parameter's `$ref` sites therefore proves the parameter is *accepted*, not that the response *carries* the field. The same caution applies to embedded resources, whose behaviour differs per read rather than following a global rule.

- Legacy Identifier Request — opt into the transition-only `legacy_id` on the role, actor, tree, and identity reads and surface it alongside the stable identifier, so a change target's numeric identifier is obtainable without leaving the CLI, while keeping the field's opt-in, nullable, and time-limited nature visible rather than presenting it as a permanent part of the contract
  + depends-on: Role Reads
  + depends-on: Actor Read
  + depends-on: Organization Tree
  + depends-on: Actor Directory
  + depends-on: Identity Read

## Identifier Mapping from Proposal History
> Problem: Change Targets Unidentifiable from the CLI — a proposal's change can only name an existing role by its legacy numeric identifier, which no CLI read exposes, so every write touching existing governance sends the operator out to the web UI to read the number off a URL mid-flow (affects: Practitioner, AI agent)

The complement to `Legacy Identifier on Reads`, not a substitute for it — scoped to the residue that route cannot serve. `include_legacy_id` reaches the role, actor, tree, and identity reads only, and no standalone accountability, domain, or policy endpoint exists, so those elements' numeric identifiers stay unreadable (S2). The residue is now **confirmed rather than inferred**: a live probe during 075's specification found the number present on a role but absent from every one of that read's embedded accountabilities and domains. Embeds are excluded **per read rather than as a global rule** — the identity read's embedded roles do carry the number — but no embed family carries it for the three element types this route serves. What does carry them is the organization's own proposal history: `proposal list` returns `changes` inline, so one walked call harvests every numeric identifier ever authored, most labelled and with rename history intact (V5/V6). The mapping is therefore **bootstrapped, not learned**. Reads only; the server remains the judge of whether a resolved target is correct.
